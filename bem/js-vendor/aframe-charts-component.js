...
AFRAME.registerComponent('charts', {
    ...
    */
    ...
    onDataLoaded: function (file) {
        ...
        // defining the ID
        var idTag = legend_title.id;
        var subcomponentArray = 0;
        ...
        for (let point of dataPoints) {
            ...
            if(properties.type === "bar"){
                ...
                if(properties.pie_doughnut){
                    ...
                    entity = generateSlice(point, pie_angle_start, pie_angle_end, properties.pie_radius, idTag + subcomponentArray);
                }
            ...
            entity.addEventListener('mouseenter', function () {
                ...
                // Causes The flickering whenever an update is required
                ...
                updateLegendSelText(legend_properties, point, properties);
                updateLegendAllText(legend_properties, getLegendText(dataPoints, point, properties));
            ...
            entity.addEventListener('mouseleave', function () {
                ...
                // If we want to clear the selection, do so here
...
function getLegendProperties(dataPoints, properties, element){
    ...
    // NEED TO ALIGN BETTER ----------------------------------------------------
    let position_tit =      {
        x: properties.show_legend_position.x - chart_position.x, 
        y: properties.show_legend_position.y - chart_position.y, 
        z: properties.show_legend_position.z - chart_position.z - (height/2) - 0.5
    };
    let position_sel_text = {
        x: properties.show_legend_position.x - chart_position.x, 
        y: properties.show_legend_position.y - chart_position.y,       
        z: properties.show_legend_position.z - chart_position.z - (height/2)
    };
    let position_all_text = {
        x: properties.show_legend_position.x - chart_position.x, 
        y: properties.show_legend_position.y - chart_position.y,                    
        z: properties.show_legend_position.z - chart_position.z
    };
    // -------------------------------------------------------------------------
...
// Add unqiue ID so that it can be updated
...
function generateLegendSelText(legendProperties, point, properties) {
    ...
    entity.id=legendProperties.title + 'SelText';
...
function updateLegendSelText(legendProperties, point, properties){
    let value = "";
    if(properties.type === 'pie'){
        value =  point['size'];
    }else{
    ...
    // Update current entity instead of creating a new one
    let entity = document.getElementById(legendProperties.title + 'SelText');
...
function generateLegendAllText(legendProperties, text) {
    ...
    entity.id=legendProperties.title + 'AllText';
...
function updateLegendAllText(legendProperties, text) {
    let entity = document.getElementById(legendProperties.title + 'AllText');
...
function generateSlice(point, theta_start, theta_length, radius, sliceID) {
    let entity = document.createElement('a-cylinder');
    entity.setAttribute('id', sliceID);
    ...
    // Changed to imrpove aesthetics for BEM_004
    entity.setAttribute('side', 'single');
    entity.setAttribute('open-ended', 'false');
